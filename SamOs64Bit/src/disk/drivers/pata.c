// Lecture 188 - PATA (parallel ATA / IDE) disk driver. The
// kernel probes PCI for an IDE controller; if present we mount
// a single primary drive and expose it through the L184 disk
// driver vtable.
...
static bool pata_pci_device_present(void){
    ...
    for(size_t i = 0; i < total_pci; i++){
        ...
        if(device->class.base == PATA_PCI_BASE_CLASS &&
           device->class.subclass == PATA_PCI_SUBCLASS){
            return true;
        }
    }
    return false;
...
// Lecture 188 - allocate per-disk private data. `type` selects
// PRIMARY / SECONDARY / PARTITION; primary_disk is the REAL
// disk a partition is hosted on (NULL for REAL disks).
//
// Upstream bug preserved verbatim: the body unconditionally
// stores `PATA_PRIMARY_DRIVE` rather than the `type` arg. Fixed
// in a future commit; SamOs keeps the bug so audit trail
// matches.
struct pata_driver_private_data* pata_private_new(PATA_DISK_DRIVE type,
                                                  struct disk* primary_disk){
    struct pata_driver_private_data* primary_drive_data =
        kzalloc(sizeof(struct pata_driver_private_data));
...
int pata_disk_base_drive_address(struct disk* disk, int offset){
    ...
    struct pata_driver_private_data* private_data =
        (struct pata_driver_private_data*)disk_private_data_driver(disk);
    if(!private_data){
...
// Lecture 188 / 189 - control-port resolver. Upstream's L188
// committed this with `struct disk disk` (by-value), then
// passed `disk` to `disk_private_data_driver` which expects a
// pointer. The mismatch would not compile. L189 fixes the arg
// to `struct disk* disk`. SamOs lands the L189 fix here at
// L188 so the file actually builds, and documents the upstream
// state.
...
int pata_driver_mount(struct disk_driver* driver){
    ...
    struct pata_driver_private_data* primary_drive_data =
        pata_private_new(PATA_PRIMARY_DRIVE, NULL);
    if(!primary_drive_data){
        res = -ENOMEM;
        goto out;
    }
    ...
    res = disk_create_new(driver, NULL, SAMOS_DISK_TYPE_REAL, 0, 0,
                          KERNEL_PATA_SECTOR_SIZE, primary_drive_data,
                          &primary_drive_data->real_disk);
    if(res < 0){
        goto out;
    }
...
int pata_driver_mount_partition(struct disk* disk, long starting_lba, long ending_lba,
                                struct disk** disk_partition_out){
    int res = 0;
    struct pata_driver_private_data* private_data =
        pata_private_new(PATA_PARTITION_DRIVE, disk);
    if(!private_data){
        res = -ENOMEM;
        goto out;
    }
    res = disk_create_new(disk->driver, disk, SAMOS_DISK_TYPE_PARTITION,
                          starting_lba, ending_lba, disk->sector_size,
                          private_data, disk_partition_out);
out:
    return res;
}
