// Lecture 78 - generic dynamic array.
//
// Stores elements of arbitrary size in a contiguous memory
// region. Grows by `t_reserved_elements` whenever capacity
// is reached.
//
// SamOs DEVIATION: depends on `krealloc` which L80 will add.
// This file is NOT in the build at L78; it's staged source.
...
int vector_resize(struct vector* vec, size_t total_needed_elements){
    ...
    size_t final_total_elements_required =
        vec->t_reserved_elements + total_elements_required;
...
void vector_reorder(struct vector* vec, VECTOR_REORDER_FUNCTION reorder_function){
    ...
    for(size_t i = 0; i < count - 1; i++){
        for(size_t j = 0; j < count - i - 1; j++){
            if(vector_at(vec, j,     elem1, vec->e_size) < 0) goto cleanup;
            if(vector_at(vec, j + 1, elem2, vec->e_size) < 0) goto cleanup;
            if(reorder_function(elem1, elem2) > 0){
                if(vector_overwrite(vec, j,     elem2, vec->e_size) < 0) goto cleanup;
                if(vector_overwrite(vec, j + 1, elem1, vec->e_size) < 0) goto cleanup;
            }
        }
    }
cleanup:
...
int vector_pop_element(struct vector* vec, void* mem_val, size_t size){
    ...
    // Shift left.
    uint8_t* ptr_del  = (uint8_t*)vec->memory + (index_to_remove * size);
    uint8_t* ptr_next = ptr_del + size;
    size_t total_to_copy = ((vec->t_elems - index_to_remove) - 1) * size;
    memcpy(ptr_del, ptr_next, total_to_copy);
    vec->t_elems--;
out:
    kfree(tmp_mem);
    return res;
