...
// fat_directory_item attribute bitmask
...
struct fat_header {
    ...
    uint32_t hidden_setors;   // book typo preserved per project convention
...
static int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector){
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != SAMOS_ALL_OK){
        res = -EIO;
        goto out;
    }
    ...
    while(1){
        ...
        // Item is unused but not the end-of-list marker.
...
int fat16_resolve(struct disk* disk){
    ...
    if(diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != SAMOS_ALL_OK){
        res = -EIO;
        goto out;
    }
    ...
    if(fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != SAMOS_ALL_OK){
        res = -EIO;
        goto out;
    }
...
static struct fat_item* fat16_new_fat_item_for_directory_item(struct disk* disk, struct fat_directory_item* item){
    ...
    // Book bug preserved: unconditional FILE override clobbers a directory
    // tag set just above. Fine while every entry we hit lives in the root
    // directory; will need a g-fix once subdirectory traversal happens.
...
int fat16_root_dir_total(struct disk* disk){
    if(!disk || !disk->fs_private){
        return -1;
    }
    struct fat_private* fat_private = disk->fs_private;
    return fat_private->root_directory.total;
}

// Sizeof self-check so a regression in struct packing is caught at runtime.
// Returns 1 if every packed struct matches its on-disk size; 0 otherwise.
int fat16_check_sizes(){
    return sizeof(struct fat_header)          == 36 &&
           sizeof(struct fat_header_extended) == 26 &&
           sizeof(struct fat_directory_item)  == 32;
}
