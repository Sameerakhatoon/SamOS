...
void process_userland_pointer_release(struct process* process, void* userland_ptr){
    ...
    vector_pop_element(process->kernel_userland_ptrs_vector,
                       &userland_ptr, sizeof(userland_ptr));
    kfree(userland_ptr);
}

bool process_userland_pointer_registered(struct process* process, void* userland_ptr){
    size_t total = vector_count(process->kernel_userland_ptrs_vector);
    for(size_t i = 0; i < total; i++){
        struct userland_ptr* current_ptr = NULL;
        vector_at(process->kernel_userland_ptrs_vector, i,
                  &current_ptr, sizeof(current_ptr));
