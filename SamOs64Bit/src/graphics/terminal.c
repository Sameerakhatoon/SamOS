// Lecture 96 - text terminal part 1.
//
// First half of the terminal module. Source-only at this lecture
// because the L96 file calls terminal_background_save and
// terminal_free, which do not exist yet. L97 lands them and
// flips terminal.o into FILES.
...
inline static size_t terminal_abs_y_for_next_character(struct terminal* terminal){
    return terminal->bounds.abs_y
         + terminal->text.row * terminal->font->bits_height_per_chracter;
}
...
// Forward decls that L97 will define.
extern void terminal_background_save(struct terminal* terminal);
extern void terminal_free(struct terminal* terminal);
...
struct terminal* terminal_create(struct graphics_info* graphics_info,
    ...
    // Capture the pixels currently behind the terminal area so
    // close-time restore is possible. Implementation lands in
    // L97.
